## ir
# lpips_wd
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_3/checkpoints/'model-epoch=028.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_3/checkpoints/'model-epoch=028.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# mse_wd
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_5/checkpoints/'model-epoch=019.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_5/checkpoints/'model-epoch=019.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# fss_scale=20_wd
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_9/checkpoints/last.ckpt --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
# fss_scale=10_wd
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_11/checkpoints/last.ckpt --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
# fss_scale=20
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_13/checkpoints/'model-epoch=012.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
# fss_scale=10
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_18/checkpoints/'model-epoch=015.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
# fss_scale=5
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_20/checkpoints/'model-epoch=017.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results

# mse_lpips_naive
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_26/checkpoints/'model-epoch=032.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results

# fss_lpips_equal
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_31/checkpoints/'model-epoch=038.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_31/checkpoints/'model-epoch=038.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# mse_lpips
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_32/checkpoints/'model-epoch=044.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_32/checkpoints/'model-epoch=044.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# fss_lpips
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_33/checkpoints/'model-epoch=041.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_33/checkpoints/'model-epoch=041.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# fss_mse
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_36/checkpoints/'model-epoch=043.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_36/checkpoints/'model-epoch=043.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# fss_scale=1_lpips
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_39/checkpoints/'model-epoch=027.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_39/checkpoints/'model-epoch=027.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# mse
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_40/checkpoints/'model-epoch=043.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_40/checkpoints/'model-epoch=043.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# fss_lpips_mse - still improving, can run more!
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_42/checkpoints/'model-epoch=043.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_42/checkpoints/'model-epoch=043.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/additional_cfg_csi.yaml --results-dir /media/leah/results

# lpips
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging/our_logs/version_45/checkpoints/'model-epoch=043.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/ir_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results

## vis
# fss_lpips_mse
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/train_cuboid_ims.py --ckpt /media/leah/logging_vis/our_logs/version_1/checkpoints/'model-epoch=085.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/cuboid_transformer/ims/vis_experiments_configs/additional_cfg.yaml --results-dir /media/leah/results_vis

## baselines
# rainformer
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/baselines/rainformer/train_rainformer_ims.py --ckpt /media/leah/logging_rainformer/our_logs/version_2/checkpoints/'model-epoch=029.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/baselines/rainformer/additional_cfg.yaml --results-dir /media/leah/results_rainformer

# predrnn
/home/theator/.pyenv/versions/leah/bin/python /home/theator/leah/cloud-forecasting-transformer/scripts/baselines/predrnn/train_predrnn_ims.py --ckpt /media/leah/logging_predrnn/our_logs/version_2/checkpoints/'model-epoch=012.ckpt' --test=True --cfg /home/theator/leah/cloud-forecasting-transformer/scripts/baselines/predrnn/additional_cfg.yaml --results-dir /media/leah/results_predrnn
